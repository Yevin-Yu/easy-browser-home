<template>
    <div class="setting">
        ...
        <div class="user-info">
            欢迎！<span>{{ username }}</span><el-button @click="loginOut" type="primary" link> 退出 </el-button>
        </div>
        <div class="content">
            <div class="item">
...
const username = ref('')
const bg_name = ref('暂无背景图')
onMounted(() => {
    axios.get('https://api.yuwb.cn/user/info',).then(res => {
        if (res.status === 200) {
            username.value = res.data.username
        }
    })
    axios.get('https://api.yuwb.cn/user/bg/name',).then(res => {
        if (res.status === 200) {
...
function onFileChange() {
    ...
    if (file) {
        ...
        axios.post('https://api.yuwb.cn/upload/bg', formData, {
            headers: {
